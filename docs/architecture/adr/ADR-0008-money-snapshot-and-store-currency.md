...
### Decision 7: HTTP Amounts Stay Major-Unit Decimals (Phase 17)

- **Decision**: OpenAPI and read DTOs expose monetary fields as major-unit decimals (`199.99`) plus an ISO 4217 `currency` code. The domain `Money` value object rounds to cents internally (`Math.round(amount * 100) / 100`). `Money.fromCents()` remains an adapter helper only; HTTP contracts do not switch to integer minor units.
- **Rationale**: Clients already format `amount` + `currency`. Integer cents would break existing storefront and admin clients without MVP benefit. Stripe and FX adapters can translate at the boundary in Phase 18+.

### Decision 8: Explicit MVP Shipping Cost (Phase 17)

- **Decision**: Cart and order read models expose `subtotal`, `shippingCost`, and `totalAmount`/`totalPrice`. MVP shipping policy is `$0` returned explicitly by the API, not invented in storefront UI copy.
- **Rationale**: `OrderPricing.calculate()` already persisted zero shipping in the domain; Phase 17 makes that fact visible on HTTP so checkout UIs cannot claim "Free" without API authority.
