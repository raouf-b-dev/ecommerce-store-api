...
1. **Anomaly prevention**: Redundant data creates the possibility for inconsistent state. If a user's address is stored in both the `orders` and `users` tables, updating one without the other creates a contradiction: the system asserts two different truths simultaneously (Codd, 1970; Kent, 1983).
...
3. **Write optimisation**: A normalised schema minimises the number of rows and indexes that must be updated when a fact changes. Updating a user's name requires modifying exactly one row in a normalised schema, versus potentially thousands of denormalised order rows.
...
> Source: Kent, William. "A Simple Guide to Five Normal Forms in Relational Database Theory." Communications of the ACM, 1983.
...
> Source: Elmasri & Navathe, _Fundamentals of Database Systems_ (7th ed., 2015), Ch. 16
...
| :-------------------------------- | :---------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------- |
| **Duplicated column**             | Copy a frequently read column from a related table into the referencing table.                              | Store `user_name` on the `orders` table alongside `user_id`.                                                                                     | Stale data if the source changes and the copy is not updated.                                                                       |
| **Pre-computed aggregate**        | Store a derived count, sum, or average on the parent row instead of computing it at query time.             | `orders_count` column on the `users` table.                                                                                                      | Count drifts if increment/decrement logic has bugs or race conditions.                                                              |
...
| **Snapshot / point-in-time copy** | Freeze a value at the moment of an event, preserving the historical state even if the source later changes. | Store `shipping_contact_name` on an order shipment at creation time, preserving who handled the shipment even if that user's name later changes. | Not actually "stale": this is intentional historical preservation. Must be clearly documented as a snapshot, not a live reference. |
| **Materialised view**             | A DBMS-managed pre-computed query result, refreshed on a schedule or on demand.                             | `CREATE MATERIALIZED VIEW order_summary AS SELECT ...`                                                                                           | Staleness window between refreshes; storage overhead.                                                                               |
...
> Source: Adapted from Date, C.J., _An Introduction to Database Systems_ (8th ed., 2003)
...
- The `shipping_contact_name` on an order shipment record is not a cached copy of the user's current name: it is a record of **who handled shipment at that moment**.
- The `order_total` on an order is not a cached sum of line items: it is the **contractual total agreed at the time of purchase**.
- The `billing_address_snapshot` on an invoice is not a cached copy: it is the **legal billing address at the time of invoicing**.
...
Updating a point-in-time snapshot column when the source entity changes. If an order shipment's `shipping_contact_name` is a snapshot, updating it when the user changes their name destroys the historical record. If it is intended to be a live reference, it should not be a duplicated column at all: it should be a JOIN.
