...
| **A08:2025** | **Software & Data Integrity Failures**    | ✅ **Compliant**  | Subresource integrity in CI actions (`persist-credentials: false`), distributed Redlock idempotency key validation (`@Idempotent()`), BullMQ SAGA state machine compensation locks.                                  | `src/infrastructure/idempotency/`, `src/modules/orders/`                                               |
...
1. **Distributed Idempotency Protection:** High-impact mutation routes (such as order checkout) are decorated with `@Idempotent()`. A Redis-backed Redlock store registers unique client request keys (`Idempotency-Key` HTTP header), returning cached responses for replayed requests and rejecting concurrent duplicate submissions.
...
4. **Integration & E2E Security Tests:** `npm run test:e2e` (runs `test/security-idor.e2e-spec.ts` verifying RBAC and ownership isolation against real PostgreSQL and Redis containers).
